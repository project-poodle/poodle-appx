...
CREATE TABLE `{{{global.schema_prefix}}}`.`deployment` (
    ...
    `update_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `runtime_name`),
    PRIMARY KEY (`id`)
)
CHARACTER SET utf8 COLLATE utf8_bin;
...
CREATE TABLE `{{{global.schema_prefix}}}`.`obj_status` (
    ...
    `status_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `runtime_name`, `obj_name`),
    PRIMARY KEY (`id`, `namespace`, `app_name`, `runtime_name`)
)
CHARACTER SET utf8 COLLATE utf8_bin
PARTITION BY KEY(`namespace`, `app_name`, `runtime_name`) PARTITIONS 20;
...
CREATE TABLE `{{{global.schema_prefix}}}`.`relation_status` (
    ...
    `status_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `runtime_name`, `obj_name`, `objn_name`),
    PRIMARY KEY (`id`, `namespace`, `app_name`, `runtime_name`)
)
CHARACTER SET utf8 COLLATE utf8_bin
PARTITION BY KEY(`namespace`, `app_name`, `runtime_name`) PARTITIONS 20;
...
CREATE TABLE `{{{global.schema_prefix}}}`.`attr_status` (
    ...
    `status_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `runtime_name`, `obj_name`, `attr_name`),
    PRIMARY KEY (`id`, `namespace`, `app_name`, `runtime_name`)
)
CHARACTER SET utf8 COLLATE utf8_bin
PARTITION BY KEY(`namespace`, `app_name`, `runtime_name`) PARTITIONS 20;
...
CREATE TABLE `{{{global.schema_prefix}}}`.`api_status` (
    ...
    `status_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `runtime_name`, `obj_name`, `api_method`, `api_endpoint`),
    PRIMARY KEY (`id`, `namespace`, `app_name`, `runtime_name`)
)
CHARACTER SET utf8 COLLATE utf8_bin
PARTITION BY KEY(`namespace`, `app_name`, `runtime_name`) PARTITIONS 20;
...
CREATE TABLE `{{{global.schema_prefix}}}`.`transform_status` (
    ...
    `status_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `runtime_name`, `obj_name`, `transform_name`),
    PRIMARY KEY (`id`, `namespace`, `app_name`, `runtime_name`)
)
CHARACTER SET utf8 COLLATE utf8_bin
PARTITION BY KEY(`namespace`, `app_name`, `runtime_name`) PARTITIONS 20;
...
-- ui model --
CREATE TABLE `{{{global.schema_prefix}}}`.`ui_app` (
    `id`                    BIGINT                  NOT NULL AUTO_INCREMENT,
    `namespace`             VARCHAR(32)             NOT NULL,
    `app_name`              VARCHAR(15)             NOT NULL,
    `ui_app_ver`            VARCHAR(32)             NOT NULL,
    `ui_app_spec`           JSON                    NOT NULL,
    `create_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `update_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `ui_app_ver`),
    PRIMARY KEY (`id`)
)
CHARACTER SET utf8 COLLATE utf8_bin;

CREATE TABLE `{{{global.schema_prefix}}}`.`ui_app_status` (
    `id`                    BIGINT                  NOT NULL AUTO_INCREMENT,
    `namespace`             VARCHAR(32)             NOT NULL,
    `app_name`              VARCHAR(15)             NOT NULL,
    `ui_app_ver`            VARCHAR(32)             NOT NULL,
    `ui_app_status`         JSON                    NOT NULL,
    `status_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `ui_app_ver`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `{{{global.schema_prefix}}}`.`ui_deployment` (
    ...
    `update_time`           TIMESTAMP               NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    `deleted`               TINYINT(1)              NOT NULL DEFAULT 0,
    UNIQUE INDEX `unique_idx`(`namespace`, `app_name`, `runtime_name`, `ui_app_ver`),
    PRIMARY KEY (`id`)
)
CHARACTER SET utf8 COLLATE utf8_bin;
