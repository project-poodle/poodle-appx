const _ = module.lodash // Import the entire lodash library
...
function load_from_local_storage() {
  for (let key of Object.keys(window.localStorage)) {
    ...
    let matches_token = key.match(new RegExp(`^/appx/(${REGEX_VAR})/(${REGEX_VAR})/userToken$`))
    if (matches_token) {
    ...
    let matches_info = key.match(new RegExp(`^/appx/(${REGEX_VAR})/(${REGEX_VAR})/userInfo$`))
    if (matches_info) {
